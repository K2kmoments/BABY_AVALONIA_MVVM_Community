...
public partial class PooPooViewModel : ViewModelBase
{
    ...
    private string _peeItemString = "";
    private string _pooItemString = "";
    ...
    {
        var peeSelect = new PooPooItem("PeePee");
        
        if((PooPooItemList?.Count + 2) <= maxItemsInCollection)
        {
            PooPooItemList?.Add(peeSelect);
            _peeItemString = "PeePee";
        }
       
    }

    [RelayCommand]
    private void BrownPooSelect()
    {
        var pooSelect = new PooPooItem("Brown Poo Poo");
        
        if((PooPooItemList?.Count + 1) <= maxItemsInCollection)
        {
            PooPooItemList?.Add(pooSelect);
            if(_pooItemString == "") _pooItemString = "BrownPoo";
        }
    }

    [RelayCommand]
    private void GreenPooSelect()
    {
        var pooSelect = new PooPooItem("Green Poo Poo");
        
        if(PooPooItemList != null && (PooPooItemList.Count + 1) <= maxItemsInCollection)
        {
            PooPooItemList.Add(pooSelect);
            if(_pooItemString == "") _pooItemString = "GreenPoo";
        } 
    }

    [RelayCommand]
    private void OrangePooSelect()
    {
        var pooSelect = new PooPooItem("Orange Poo Poo");
        
        if(PooPooItemList != null && (PooPooItemList.Count + 1) <= maxItemsInCollection)
        {
            PooPooItemList.Add(pooSelect);
            if(_pooItemString == "") _pooItemString = "OrangePoo";
        } 
    }

    [RelayCommand]
    private void YellowPooSelect()
    {
        var pooSelect = new PooPooItem("Yellow Poo Poo");
        
        if(PooPooItemList != null && (PooPooItemList.Count + 2) <= maxItemsInCollection)
        {
            PooPooItemList.Add(pooSelect);
            if(_pooItemString == "") _pooItemString = "YellowPoo";
        } 
    }
    ...
    private void SaveButtonPress()
    {
        ...
        var entryToSave = new LogItemTemplate()
        {
            ...
            Note = $"In the Diaper was:{_peeItemString}; {_pooItemString}",
            LogCreationDateTime = DateTime.Now,
            IconString = _iconStringPooPoo
        };
        if (_pooItemString != null)
        {
